...
* @author Mohamed Tayeh
*/
const updateIsoCodeIncomeLevelQueries = (isoCodeData, incomeLevelToID, isoCodeToID) => {
...
module.exports = {
    updateIsoCodeData: async (args) => {
        ...
        let incomeToID = Promise.all(incomeLevelQueries).then(res => {
        ...
        let isoCodeTypeToID = Promise.all(isoCodeTypeQueries).then(res => {
        ...
        let isoCodeToID = Promise.all(isoCodeQueries).then(res => {
